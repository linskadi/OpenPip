# 竞赛论文写作规范

## 论文结构规范

### 标准章节及篇幅

| 章节 | 篇幅占比 | 核心功能 |
|------|---------|---------|
| 摘要 | 1页 | 最重要！概述问题、方法、结果、结论 |
| 一、问题重述 | 8-12% | 问题背景、重述、研究内容概述 |
| 二、问题分析 | 8-12% | 每个问题的分析思路 |
| 三、模型假设 | 适量 | 假设 + Justification |
| 四、符号说明与数据预处理 | 适量 | 符号表 + EDA描述性统计 |
| 五、模型的建立与求解 | 50-60% | 各问题的模型构建与结果 |
| 六、敏感性分析 | 4-8% | 参数敏感性与稳健性验证 |
| 七、模型评价 | 4-8% | 优势与局限性讨论 |
| 参考文献 | 1页 | 引用文献 |

## 段落式写作

**严格禁止分点式论述（bullet points / numbered lists）出现在正文中。**

必须将分点式内容转换为流畅的论文级段落式语言。

❌ 错误示例：
```
关键发现：
1. 右偏分布：大多数国家奖牌数较少
2. 均值 > 中位数：数据被高奖牌数国家拉高
3. 异常值：超级强国奖牌数远高于平均水平
```

✅ 正确示例：
```
分布分析揭示了奖牌数据的若干显著特征。大多数国家获得的奖牌数较少，
中位数仅为5枚，而少数强国累积了显著更高的总量，形成了右偏分布。
均值超过中位数进一步证实了这一点，表明平均值被高表现国家抬高。
此外，美国和前苏联等国家的奖牌数远超典型水平，构成统计意义上的异常值。
```

## 过渡连接词

| 类型 | 过渡词 |
|------|--------|
| 递进 | Furthermore, Moreover, Additionally, In addition |
| 因果 | Therefore, Thus, Consequently, As a result |
| 转折 | However, Nevertheless, In contrast |
| 举例 | For example, Specifically, As illustrated by |
| 总结 | In summary, To conclude, Overall |

## 语态与时态

- 被动语态为主：强调客观性 (The model was trained using...)
- 主动语态：强调研究贡献 (We develop a novel framework...)
- 现在时：描述模型、公式、一般性结论
- 过去时：描述实验过程、数据处理步骤

## 写作禁忌

- 避免口语化表达（"a lot of" → "numerous"）
- 避免主观评价词（"very good" → 用数据支撑）
- 避免过长句子（每句不超过30词）
- 禁止分点列表出现在正文论述中

---

## 各章节写作要点

### 摘要（最重要！）

结构：背景(1-2句) → 方法(2-3句) → 结果(2-3句) → 结论(1-2句)

- 每个问题单独成段，说明用了什么模型、如何构建、得到什么结果（含具体数值）
- 最后总结敏感性分析使用和效果
- 关键词 4-5个
- 不要出现复杂公式和表格
- 不要只重述题目
- 不要用"我们认为……""本文将……"等空泛表述
- 中国赛摘要一般不超过两页，不需要翻译成英文
- 美赛 Summary Sheet 是独立一页，应包含方法亮点、关键结论和模型创新点

### 问题分析

每个问题的分析必须包含：
- 问题属于什么类型（预测/分类/优化/评价）
- 为什么选择该模型（与备选方案的对比）
- 分析思路概述

### 模型假设

每个假设必须附 Justification：

```
假设1：[假设内容]
合理性说明：[为什么这样假设是合理的，结合领域知识论证]
```

假设分类：
1. **必要假设**：建模必须的前提（如"忽略空气阻力"）
2. **简化假设**：降低问题复杂度（如"物体为质点"）
3. **理想化假设**：现实不可能但数学上可行（如"线性关系"）

### 模型章节（核心）

每个问题的标准结构：
1. 问题分析
2. 模型构建（含公式推导，公式参数必须写明从何而来）
3. 求解方法
4. 结果展示（图表，每张图至少配3行分析）
5. 结果分析

#### 参数定量依据（关键检查项）

所有模型参数都必须有明确来源，禁止出现无解释的"默认值"：
- 参数来源三选一：数据统计 / 文献引用 / 校准实验
- 阈值选择需有敏感性分析或样本量说明

#### 模型选择论证

选择任何模型必须包含：
1. 为什么选这个模型（1-2句）
2. 与备选方案的对比（表格或段落）

#### 因果 vs 相关区分

预测准确率高 ≠ 因果关系！必须明确声明：

```
While our statistical tests establish that [pattern] exists,
this does not prove causation. Alternative explanations include [alternatives].
```

### 敏感性分析

写作骨架：
1. 参数选择与范围：指出关键参数及取值范围
2. 性能与稳健性评估：评价指标与对比结论
3. 可视化与表格支撑

方法：单参数扰动（±10%、±20%）；Tornado 图展示参数影响力排序。

### 模型评价

- 优点数量多于缺点
- 缺点约2-3个
- 每个优缺点都要有具体依据
- 不要只说优点，必须客观讨论缺点

---

## 图片插入规范

**必须在论文中插入代码生成的图片。** 仅仅在文字中写"如图X所示"是不够的，必须同时插入图片标签。

### 插入格式

每张图片必须使用以下格式单独一行插入：

```markdown
![图片描述](文件名.png)
```

### 插入规则

1. **收到的可用图片列表中的每张图片都必须在论文中插入**
2. 图片标签必须独占一行，放在相关段落之后
3. 使用图片的**原始文件名**，不要修改文件名
4. 在图片标签之前或之后，必须有至少3行文字对图片内容进行解读分析
5. 禁止只写"如图所示"而不插入图片标签

### 正确示例

```
相关性分析结果表明，母亲EPDS得分与婴儿睡眠时长呈显著负相关（r=-0.35, p<0.01），
HADS焦虑得分与睡醒次数呈正相关（r=0.28, p<0.05）。

![相关性热力图](fig1_correlation_heatmap.png)

从热力图可以观察到，母亲心理指标之间也存在较强的共线性关系...
```

### 错误示例（禁止）

```
如图1所示，相关性分析表明...（没有插入![](filename.png)标签）
```

---

## 图表解读规范

### 引用句式

- 引入：As illustrated in Fig. X, [图表核心内容].
- 解读：Fig. X shows/depicts/demonstrates that [趋势/关系].
- 对比：Compared with [对比对象], [研究对象] achieves [结果].

### 图表解读规则

- **每幅图表至少配3行分析**，说明趋势、原因、与结论的关系，禁止"只放图不解释"
- 图表描述必须基于代码输出的实际数据特征，禁止编造数值

---

## 公式与格式排版

### LaTeX 格式

- 行内公式：`$...$`
- 块级公式：`$$...$$`
- 表格使用 markdown 语法

### 引用协议

1. **每个参考文献在全文中只能被引用一次**
2. 引用格式：`[^N] 完整引用信息`
3. 从 `[^1]` 开始唯一编号，依次递增
4. 引用用花括号包裹整个引用
5. 添加引用前检查是否已引用过，已引用的不再重复
6. 理论性章节必须搜索文献支持

---

## 常见写作错误防错清单

- ❌ 假设没有在正文建模处引用 → 假设形同虚设，评委质疑合理性
- ❌ 结论只给数值，不解释含义 → "最优值为 1234.56"而不说明对应什么方案、有什么业务含义
- ❌ 图表出现但正文没有引导和解释 → 评委不知道图要说明什么
- ❌ 连续三张以上图表中间没有文字 → 论文变成图集
- ❌ 公式符号首次出现处没有定义 → 读者无法理解模型
- ❌ 参考文献是捏造或无法核实的 → 严重信誉问题
- ❌ 摘要和正文数值不一致 → 摘要最后写、从正文复制数值可避免

---

## 执行约束

1. 自主完成写作，不要询问过程性问题
2. 输出纯 Markdown/LaTeX 内容，不要加代码块标记
3. 严格使用图片的实际文件名引用
4. 保持与用户输入一致的语言
5. **绝不重复引用**：每个参考文献只出现一次

---

## 摘要写作要点（补充）

- 摘要是评委最先也最重视的部分，应最后撰写（内容全部确定后）。
- 每个子问题必须覆盖：用了什么方法、建立了什么模型、主要结果是什么（含关键数值）。
- 不要出现复杂公式和表格；不要只重述题目。
- 中国赛摘要一般不超过两页，不需要翻译成英文。
- 美赛 Summary Sheet 是独立一页，应包含方法亮点、关键结论和模型创新点。

## 论文结构完整性检查清单

完整的竞赛论文必须包含以下 9 个部分：

1. **摘要**：方法 + 结论 + 数值结果
2. **问题重述与分析**：建模者的理解和重新表达（不是照抄题面）
3. **模型假设**：每条假设必须有必要性说明和正文引用
4. **符号说明**：所有后文用到的符号在此定义，有量纲标注
5. **模型建立与求解**：每个子问题独立成节
6. **结果分析与检验**：数值结果 + 约束验证 + 误差分析
7. **灵敏度分析**：关键参数扰动时结果变化
8. **模型评价与推广**：优缺点、改进方向
9. **参考文献**：真实存在、格式规范

## 灵敏度分析规范

- 灵敏度分析不是可选项，是竞赛论文的必要内容。
- 方法：单参数扰动（±10%、±20%）观察目标函数变化；或 Tornado 图展示参数影响力排序。
- 对优化类问题：改变约束右端项或目标系数，观察最优解是否发生质变。
- 对预测类问题：改变关键假设或初始条件，观察预测曲线漂移幅度。
- 结论：若小扰动导致结果大幅变化，说明模型对该参数敏感，需在论文中声明。

## 常见写作错误防错清单（补充）

- ❌ 假设没有在正文建模处引用 → 假设形同虚设
- ❌ 结论只给数值，不解释含义
- ❌ 图表出现但正文没有引导和解释
- ❌ 连续三张以上图表中间没有文字 → 论文变成图集
- ❌ 公式符号首次出现处没有定义
- ❌ 参考文献是捏造或无法核实的 → 严重信誉问题
- ❌ 摘要和正文数值不一致 → 摘要最后写可避免
- ❌ 内部文件名、脚本名、工作流说明出现在正文中 → AI痕迹应去除
